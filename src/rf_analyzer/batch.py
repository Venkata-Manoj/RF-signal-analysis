...
def analyze_folder(
    ...
    *,
    ...
    Args:
        ...
        decode_max_bits: Bound on the decode search prefix (``None`` keeps the
            pipeline default; correctness tests pass a generous value so a
            loaded machine cannot decide the result).
        decode_time_budget_s: Wall-clock ceiling for the decode search
            (``None`` keeps the pipeline default; same generosity rule).
    ...
    for index, path in enumerate(captures, start=1):
        ...
        if decode_max_bits is not None:
            request["decode_max_bits"] = int(decode_max_bits)
        if decode_time_budget_s is not None:
            request["decode_time_budget_s"] = float(decode_time_budget_s)
...
#: Cell prefixes that spreadsheet apps interpret as formulas (OWASP CSV
#: Injection). Any string cell whose left-stripped text starts with one of
#: these is prefixed with a single quote on export so Excel/LibreOffice treat
#: it as text.
CSV_FORMULA_PREFIXES = ("=", "+", "-", "@", "|", "%")


def sanitize_csv_cell(value: Any) -> Any:
    """Neutralise a single CSV cell against formula injection (OWASP).

    Non-strings pass through untouched. A string whose ``lstrip()`` starts
    with ``= + - @ | %`` is prefixed with ``'`` so the spreadsheet treats it
...
def write_csv(rows: Iterable[dict[str, Any]], path: str | Path) -> Path:
    ...
    with open(out, "w", encoding="utf-8-sig", newline="") as handle:
        ...
        writer.writeheader()
        for row in rows:
            writer.writerow({k: sanitize_csv_cell(row.get(k, "")) for k in keys})
    return out
...
def render_html(
    ...
    parts.append(
        "<footer>Generated by the RF Signal Analysis Workbench.</footer>"
        "</body></html>"
    )
    return "\n".join(parts)
